...
class Ordercontroller extends Controller
{
    ...
    {
        // Compute basket total price (if not already included in $summary)
        $basketTotal = collect($basket)->sum('total_price');

        $emailData = [
            'customer'     => $order, // Order details
            'summary'      => $summary, // Order summary (includes total_price key)
            'basket'       => $basket,  // Detailed basket items
            'basket_total' => $basketTotal, // Basket total to be used in your view
        ];
